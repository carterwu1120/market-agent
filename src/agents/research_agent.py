...
# Real MCP tool-calling via `claude -p --mcp-config` (src/mcp_server.py exposes
# the same 11 tools). Claude Code runs its own agentic loop; we just parse the
# symbols it touched out of the final text for state persistence.
...
async def _run_claude_code_research(state: AgentState) -> dict:
    ...
    logger.info(f"ResearchAgent(claude_code): done, symbols={used_symbols}")
