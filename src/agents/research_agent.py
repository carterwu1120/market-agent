...
公司長期競爭力調查規則：
- 使用者詢問長期價值、護城河、獨有技術、供應鏈地位或產品競爭力時，考慮呼叫 company_moat_analysis。
- 新聞若出現新技術、新產品、專利、量產、訂單、客戶認證或關鍵供應鏈事件，
  也應考慮調查；本輪已有明確新事件時可用 refresh=true。
- 純報價、一般技術面或短線問題不要呼叫，避免無效搜尋。
- 新技術不等於獨有、進入供應鏈不等於不可替代、好公司不等於目前價格值得買。
- 結論區分「已有證據的優勢」「發展中、待量產驗證」「僅題材」「證據不足」，並附來源與主要反證。
"""
...
公司競爭力調查只用於長期持倉，或新聞明確涉及技術、量產、客戶、認證與供應鏈事件的全新觀察標的。
一般短線輪詢不必呼叫 company_moat_analysis；若資料包已有 company_moat_evidence，
應視為待查證線索而非已證實護城河。
"""
