...
┌─────────────────────────────────────────┐
│             Orchestrator                │
│  LLM 分類 intent + rule-based 補強      │
│  intent: daily_brief / stock_query /    │
│          sector_query / theme_query /   │
│          research / follow_up           │
└────────────────┬────────────────────────┘
                 │
        ┌────────┴────────┐
        │                 │
        ▼                 ▼
 intent = research    其他 intent
 (複雜/比較型問題)    (一般查詢)
        │                 │
        ▼                 │ fan-out（並行執行）
┌───────────────┐         ├── news_agent      （新聞爬蟲）
│ research_agent│         ├── technical_agent  （技術面 + 法說會）
│  ReAct Loop   │         ├── fundamental_agent（基本面）
│               │         ├── chip_agent       （三大法人）
│ LLM 自主決定  │         ├── social_agent     （PTT訊號）
│ 呼叫哪些工具  │         └── rag_agent        （向量搜尋）
│  ┌─────────┐  │                 │
│  │ToolNode │  │                 ▼
│  │sector_  │  │         ┌───────────────┐
│  │lookup   │  │         │  Synthesizer  │
...
└───────┬───────┘
        │
        ▼
    最終報告（research_agent 直接輸出，不經 synthesizer）
```
