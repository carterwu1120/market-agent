...
- 🧪 **紙上交易迴圈**（實驗性）— 交易時間內背景自動運作；程式固定並行抓技術面、基本面、籌碼、公告與新聞，每輪最多兩檔，再交給 LLM 做一次結構化 JSON 決策，由風控層驗證後才交給 PaperBroker。短線每 30 分鐘輪替，長線候選／持倉依基本面與公司競爭力評分、預設每 7 天檢視；機械式停損與條件單每分鐘獨立運作。完整設計見 [`docs/paper_trading.md`](docs/paper_trading.md)
