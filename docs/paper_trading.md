...
交易、停損、條件單、狀態頁與資產快照共用程序內行情層。同一股票預設 15 秒內只向供應商查詢一次；同時到達的請求會等待同一個查詢結果。Yahoo 的交易行情路徑只讀短期歷史價格，不再為每次輪詢載入完整公司資料。

- `QUOTE_CACHE_TTL_SECONDS`：新鮮行情的共用時間，預設 15 秒。
- `QUOTE_STALE_FALLBACK_SECONDS`：供應商失敗時可供顯示與估值使用的最後有效價格期限，預設 300 秒。
- 舊價會標記 `is_stale=true`；Discord、CLI、MCP 會揭露使用舊價的股票。買進、賣出、停損與價格條件禁止使用舊價。
- 條件單依實際 indicator 抓資料：只有 `close` 時不再取得技術指標，只有技術指標時不預先取得現價；法人連買資料也維持按需取得。
- 每 30 分鐘的 `equity_snapshot` 會附上 provider 請求、錯誤、快取命中、合併請求、舊價備援及平均延遲統計。統計在程序重啟後重新累積。
